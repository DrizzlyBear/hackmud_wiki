	...
	if (typeof props.children == "string") {
		return <>
			{parseColorCodes(props.children).map(token => {
				if (token.color)
					return <span style={`font-family: White Rabbit, monospace; color: ${colorCodesToHex[token.color]}`}>{token.text}</span>

				return <span style={`font-family: White Rabbit, monospace`}>{token.text}</span>
			})}
		</>
	}
